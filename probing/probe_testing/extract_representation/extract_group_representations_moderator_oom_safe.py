#!/usr/bin/env python3
"""
OOM-SAFE: Extract group-level representations for moderator cells with RESUME FROM EXISTING JSON.

CRITICAL CHANGES from batched version:
  - Load existing group_representations_moderator.json if it exists (resume from there)
...
For each cell (condition × moderator × level × outcome):
...
Usage:
  python extract_group_representations_moderator_oom_safe.py \\
    --cells_file ../prompt/cells_moderator_prompts_tier2.json \\
    --prompts_dir ../prompt \\
    --model_path /projects/p32143/cache/qwen36_27b \\
    --output_file group_representations_moderator.json \\
    --batch_size 4 \\
    --device cuda
...
log = logging.getLogger("extract_group_representations_moderator_oom_safe")
...
def main() -> None:
    ...
    parser.add_argument("--cells_file", default="../prompt/cells_moderator_prompts_tier2.json")
    parser.add_argument("--prompts_dir", default="../prompt")
    parser.add_argument("--model_path", default="/projects/p32143/cache/qwen36_27b")
    parser.add_argument("--output_file", default="group_representations_moderator.json")
    ...
    log.info("=" * 78)
    log.info("EXTRACT GROUP-LEVEL REPRESENTATIONS FOR MODERATOR (OOM-SAFE, RESUME)")
    log.info("=" * 78)
