...
# 1. Base pipeline with PCA
pipeline = Pipeline([
    ("scaler", StandardScaler()),
    ("pca", PCA(svd_solver='randomized', random_state=42)), # Enforce memory-friendly solver
    ("ridge", Ridge())
])
...
grid_search = GridSearchCV(
    ...
    cv=5,
    scoring='r2',
    # FIX 3: Throttled n_jobs from -1 to 2. Prevents 16 workers from duplicating the 16GB array
    n_jobs=2, 
    pre_dispatch='2*n_jobs', # Prevents joblib from queuing too many memory-intensive tasks in advance
    verbose=1
)
...
joblib.dump(best_pipeline, "ridge_probe_model_combined.joblib")
print(f"  Saved: ridge_probe_model_combined.joblib")
...
with open("evaluation_report_combined.json", 'w') as f:
    json.dump(report, f, indent=2)
print(f"  Saved: evaluation_report_combined.json")
